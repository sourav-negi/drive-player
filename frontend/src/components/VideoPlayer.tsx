...
export function VideoPlayer({ src, fileId, title, initialProgress }: Props) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "ArrowLeft": case "j": e.preventDefault(); p.skip(-5); break;
        case "ArrowRight": case "l": e.preventDefault(); p.skip(5); break;
        case "ArrowUp": e.preventDefault(); p.setVolume(Math.min(1, p.volume + 0.1)); break;
        case "ArrowDown": e.preventDefault(); p.setVolume(Math.max(0, p.volume - 0.1)); break;
  ...
  return (
    ...
    >
      ...
      {/* controls overlay */}
      <div
        className={`controls-overlay absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/95 via-white/80 to-transparent px-3 sm:px-4 pt-10 sm:pt-12 pb-2 sm:pb-3 transition-opacity duration-300 ${
          showControls ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
      >
        <SeekBar currentTime={player.currentTime} duration={player.duration} onSeek={player.seek} />

        <div className="flex items-center justify-between mt-1 gap-1 sm:gap-0">
          {/* left controls */}
          <div className="flex items-center gap-1 sm:gap-2">
            <button onClick={() => player.skip(-5)} className="h-8 sm:h-8 w-8 sm:w-8 flex items-center justify-center rounded text-black hover:text-gray-700 transition-colors" title="back 5s">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12.5 8c-2.65 0-5.05.99-6.9 2.6L2 7v9h9l-3.62-3.62c1.39-1.16 3.16-1.88 5.12-1.88 3.54 0 6.55 2.31 7.6 5.5l2.37-.78C21.08 11.03 17.15 8 12.5 8z" />
              </svg>
            </button>

            <button onClick={player.togglePlay} className="h-10 sm:h-10 w-10 sm:w-10 flex items-center justify-center rounded-full bg-white text-black hover:bg-gray-200 transition-colors" title={player.playing ? "pause" : "play"}>
              {player.playing ? (
            ...
            <button onClick={() => player.skip(5)} className="h-8 sm:h-8 w-8 sm:w-8 flex items-center justify-center rounded text-black hover:text-gray-700 transition-colors" title="forward 5s">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M18 13c0 3.31-2.69 6-6 6s-6-2.69-6-6 2.69-6 6-6v4l5-5-5-5v4c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8h-2z" />
              </svg>
            </button>

            <span className="text-[11px] sm:text-xs text-gray-700 ml-1 sm:ml-2 tabular-nums min-w-[70px] sm:min-w-[80px]">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button onClick={player.toggleFullscreen} className="h-8 sm:h-8 w-8 sm:w-8 flex items-center justify-center rounded text-black hover:text-gray-700 transition-colors" title="fullscreen">
